...
public class ReadFragment extends Fragment implements OnItemClickListener,OnItemLongClickListener{
	...
	/*	final ListView l=(ListView) rootView.findViewById(R.id.listView1);
     ...
     hotornot info=new hotornot(getActivity());
		info.open();
		Cursor cursor = info.getallmsg();
		String result = null;
		List list = new ArrayList();
		list.add("abc");
		 rowItems = new ArrayList<RowItemAnniversary>();
		 int i=1;
...
//result=result + cursor.getString(0) + "  " + cursor.getString(1) +"  " + cursor.getString(2)+"  "+cursor.getString(3)+" "+cursor.getString(4)+"\n";
	    	RowItemAnniversary item = new RowItemAnniversary(i, cursor.getString(0), cursor.getString(1),cursor.getString(2),cursor.getString(3),cursor.getString(4),cursor.getString(5));
	    	;
	    	rowItems.add(item);
           i++;
    ...
    // ArrayAdapter<String> adapter = new ArrayAdapter<String>(getActivity(),android.R.layout.simple_list_item_1, android.R.id.text1, list);
	    CustomBaseAdapteranni adapter = new CustomBaseAdapteranni(getActivity(), rowItems);
    l.setAdapter(adapter);
		    ...
		    public void onClick(DialogInterface dialog, int which) {
		    	...
		    	//	smsIntent.putExtra("sms_body","Body of Message");
		    		...
		    		//	Toast.makeText(getActivity(),  rowItems.get(arg2).getTitle(), Toast.LENGTH_LONG).show();
		    			info.Delete_Contactanni(k);
		    			info.close();
		    			ReadFragment.this.onResume();
						}
				    });
			    adb.show();
